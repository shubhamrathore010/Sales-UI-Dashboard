...
export const Sidebar: React.FC<SidebarProps> = ({ onClose }) => (
  <aside className="w-72 flex-shrink-0 bg-white border-r border-gray-100 flex flex-col h-full py-8 shadow-2xl lg:shadow-none">
    ...
    <div className="px-5 mt-auto">
      <button className="w-full flex items-center space-x-4 px-5 py-3.5 text-gray-500 hover:bg-[#F7F5F3] hover:text-[#1A1A1A] rounded-[16px] transition-all mb-4 group">
        <Settings className="w-[22px] h-[22px] text-gray-400 group-hover:text-[#1A1A1A] transition-colors" />
      ...
      <div className="p-5 bg-[#F7F5F3] rounded-[24px] relative overflow-hidden group cursor-pointer border border-white">
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-2">
             <span className="px-2 py-0.5 bg-[#1A1A1A] text-[9px] font-black text-white rounded-md tracking-widest uppercase">PRO PLAN</span>
          </div>
          ...
          <button className="w-full py-2.5 bg-white text-[#1A1A1A] text-xs font-extrabold rounded-xl shadow-sm hover:bg-[#E67E7E] hover:text-white transition-all transform active:scale-95">
            Upgrade Now
          </button>
        </div>
